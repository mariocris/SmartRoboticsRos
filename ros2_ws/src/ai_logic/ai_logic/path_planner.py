...
class TrajectoryActionClient(Node):
    ...
    def send_goal(self, pos):
        ...
        for response in pos:
            ...
            point_msg.positions = [response.j1,  response.j2,
                                   response.j3, response.j4, response.j5, response.j6]
...
def main(args=None):
    ...
    for ii in waypoints:
        if ii == (0, 0):
            targets.append([0.05, -0.5, Z])
        if ii == (0, 1):
            targets.append([0.5, -0.5, Z])
        if ii == (0, 2):
            targets.append([1, -0.5, Z])
        if ii == (1, 0):
            targets.append([0.05, 0.05, Z])
        if ii == (1, 1):
            targets.append([0.5, 0.05, Z])
        if ii == (1, 2):
            targets.append([1, 0.05, Z])
        if ii == (2, 0):
            targets.append([0.05, 0.5, Z])
        if ii == (2, 1):
            targets.append([0.5, 0.5, Z])
    ...
    for pos in positions:
        # rclpy --> api python for interacting with ros2
        rclpy.init()
        action_client = TrajectoryActionClient()  # initialize ActionClient
        action_client.send_request(pos)  # create request for each target position
        rclpy.spin_once(action_client)  # Execute one item of work or wait until a timeout expires (used to integrate with another thing that needs the same thread)
        response = action_client.future.result()
        if action_client.future.done():
            pos_joints.append(response)
            print("joint responses: ", response.j1, ", ",  response.j2, ", ", response.j3,
